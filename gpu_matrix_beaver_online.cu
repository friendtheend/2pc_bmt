...
//       -L/usr/lib/x86_64-linux-gnu/openmpi/lib -lmpi -o gpu_matrix_online
//
// 运行:
//   mpirun -np 2 ./gpu_matrix_online --pcg matrix_triple_party --M 64 --K 32 --N 64
...
int main(int argc, char** argv) {
    ...
    // Benchmark
    double sum_ms = 0, sum_comm = 0, sum_gemm = 0;
    
    for (int iter = 0; iter < iters; ++iter) {
        cudaEvent_t ev_start, ev_stop, ev_gemm_start, ev_gemm_stop;
        ...
        // de[M×N] = d[M×K] × e[K×N] (只有 party 0 需要)
        if (rank == 0) {
        ...
        // Phase 6: 最终计算 Y = C + dB + Ae + de
        k_beaver_final<<<blocks_linear, threads>>>(
            d_C, d_dB, d_Ae, d_de, d_Y, nC, rank == 0 ? 1 : 0);
        ...
        if (rank == 0) {
            printf("Iter %d: %.2f ms (comm: %.2f ms, GEMM: %.2f ms)\n", 
                   iter + 1, ms, comm_ms, gemm_ms);
        }
    ...
    if (rank == 0) {
        ...
        // 吞吐量: M×K×N 次乘加
        double ops = 2.0 * g_M * g_K * g_N;  // 每个 C[i,j] 需要 K 次乘和 K 次加
        double gops = ops / (avg_ms / 1000) / 1e9;
    ...
    if (do_verify) {
        ...
        if (rank == 0) {
            ...
            for (size_t s = 0; s < samples; ++s) {
                ...
                __uint128_t acc = 0;
                for (int k = 0; k < g_K; ++k) {
                    acc += (__uint128_t)(X_sum[i * g_K + k] & g_mask) * (W_sum[k * g_N + j] & g_mask);
                }
