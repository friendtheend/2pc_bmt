...
# -------------------------
ROOT       = ../..
CRYPTOLIB_DIR = $(ROOT)/cryptolib
CRYPTOLIB_BUILD = $(CRYPTOLIB_DIR)/build
MPC_COMM_DIR = $(ROOT)/mpc-comm
MPC_COMM_BUILD = $(MPC_COMM_DIR)/build
MPC_OT_DIR = $(ROOT)/mpc-ot
MPC_OT_BUILD = $(MPC_OT_DIR)/build
BUILD_DIR  = build
...
# 编译标志
# -------------------------
CXXFLAGS = -O3 -g -std=c++20 -Wall -Wextra -fopenmp -march=native -pthread -DOTX_WITH_MPI -DOTX_HAS_AESNI=1 -DOTX_HAS_VAES=1
CXXFLAGS += -I$(CRYPTOLIB_DIR)/include -I$(MPC_COMM_DIR)/include -I$(MPC_OT_DIR)/include
CXXFLAGS += -I$(MPI_INC_PATH)
...
LIBS = \
  $(MPC_OT_BUILD)/libmpc-ot.a \
  $(MPC_COMM_BUILD)/libmpc-comm.a \
  $(CRYPTOLIB_BUILD)/libcryptolib.a \
...
offline: $(BUILD_DIR)
	...
	$(MPICXX) $(CXXFLAGS) PCG_matrix_beaver.cpp -o $(OFFLINE_BIN) $(LDFLAGS) $(LIBS) $(MPI_LIBS)
