// PCG_silent.cpp - Offline Phase with Silent OT
//
// 使用 Silent OT 替代 IKNP，通信量减少约 10 倍
//
// 编译:
//   mpicxx -O3 -std=c++20 -fcoroutines -fopenmp -march=native \
//       -I/usr/local/include -DCOPROTO_ENABLE_BOOST \
//       PCG_silent.cpp -o pcg_offline [libs...]
//
// 运行:
//   mpirun -np 2 ./pcg_offline --num_triples 10000000 --output offline_party
...
// ============================================================
// Silent OT 版 Gilboa 乘法
// ============================================================

static void gilboa_batch_silent(
    u64 seed,
    coproto::Socket& sock,
    int role,
    ...
    size_t& ot_counter)
{
    ...
    for (size_t ob = 0; ob < num_ot_batches; ++ob) {
        ...
        // Cross term 1: 使用 Silent OT
        std::vector<u64> share_cross1;
        {
            int role = (party == 0) ? 0 : 1;
            if (role == 0) {
                gilboa_batch_silent(ot_seed + 1000 + ot_counter, sock, 0, chunk_a, {}, share_cross1);
            } else {
                gilboa_batch_silent(ot_seed + 1000 + ot_counter, sock, 1, {}, chunk_b, share_cross1);
            }
        }

        // Cross term 2: 使用 Silent OT
        std::vector<u64> share_cross2;
        {
            int role = (party == 0) ? 1 : 0;
            if (role == 0) {
                gilboa_batch_silent(ot_seed + 2000 + ot_counter, sock, 0, chunk_a, {}, share_cross2);
            } else {
                gilboa_batch_silent(ot_seed + 2000 + ot_counter, sock, 1, {}, chunk_b, share_cross2);
            }
        }
    ...
    const std::string& output_file)
{
    ...
    std::fprintf(stderr, "\n[Party %d] === Offline Phase (Silent OT) ===\n", party);
    ...
    std::fprintf(stderr, "\n[Party %d] === Offline Phase Complete (Silent OT) ===\n", party);
...
int main(int argc, char** argv)
{
    ...
    if (rank == 0) {
        ...
        std::fprintf(stderr, "[Info] Using Silent OT (low communication)\n");
    }
    ...
    if (rank == 0) {
        ...
        std::fprintf(stderr, "║  (Silent OT - Low Communication Mode)            ║\n");
        std::fprintf(stderr, "╠══════════════════════════════════════════════════╣\n");
